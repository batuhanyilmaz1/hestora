...
/// PNG/JPG arka plan + slot yerleşimi (`hestora_*` örnekleri ve geriye dönük kullanım).
class RasterPropertyShareCardTemplateView extends StatelessWidget {
  const RasterPropertyShareCardTemplateView({
    super.key,
    required this.theme,
    required this.data,
    required this.width,
  ...
  Widget build(BuildContext context) {
    ...
    addText(
      ShareCardThemeSlot.footerContact,
      data.footerContactLine ?? '',
      style: TextStyle(
        color: theme.subtitleColor,
        fontSize: 9.5 * scale,
        fontWeight: FontWeight.w600,
    ...
    final logoDef = theme.region(ShareCardThemeSlot.logo);
    if (logoDef != null) {
      final rect = logoDef.toRect(cardSize: size, safePadding: pad);
      final avatar = data.agentAvatarUrl?.trim() ?? '';
      if (theme.useAgentAvatarForLogo && avatar.isNotEmpty) {
        children.add(
          Positioned(
            left: rect.left,
            top: rect.top,
